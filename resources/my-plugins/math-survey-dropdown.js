...
jsPsych.plugins['survey-multi-dropdown'] = (function() {
  var plugin = {};

  plugin.info = {
    name: 'survey-multi-dropdown',
    description: '',
    parameters: {
      questions: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
        ...
        // add question text
        display_element.querySelector(question_selector).innerHTML += '<span class="' + plugin_id_name + '-text survey-multi-choice"></span>';

      questionHtml = trial.questions[i].prompt +': <select>';
      // create option radio buttons
      for (var j = 0; j < trial.questions[i].options.length; j++) {
        questionHtml += '<option value="'+trial.questions[i].options[j]+'">'+trial.questions[i].options[j]+'</option>';
      }
      display_element.querySelector(question_selector).innerHTML += questionHtml + '</select>';
    }
    // add submit button
